...
if __name__ == '__main__':
    ...
    inputs.append(grpcclient.InferInput('collision_energy_in:0',[batch_size,1],"INT32"))
    inputs.append(grpcclient.InferInput('precursor_charge_in_int:0',[batch_size,1],"INT32"))
    inputs.append(grpcclient.InferInput('instrument_indices:0',[batch_size,1],"INT64"))

    # Create the data for the two input tensors. Initialize the first
    # to unique integers and the second to all ones.
    peptide_seq_in = np.array([ ["AAAAAKAKM[UNIMOD:21]"] for i in range (0,batch_size) ], dtype=np.object_)
